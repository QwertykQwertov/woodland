...
if ($subcategory) {
  $sql = "SELECT * FROM products_stools WHERE subcategory = $subcategory";
} else {
  $sql = "SELECT * FROM products_stools";
}
  ...
  <div class="ki-product-card-wrapper d-flex justify-content-around">
    ...
    <?php foreach ($products as $product): ?>
      <div class="ki-product-card text-center">
        <a href="<?= '/catalog/stools/product?id=' . $product['id'] ?>" class="ki-product-card-link">
          <div class="ki-card-image-wrapper">
            <img class="ki-card-image" src=<?= '/assets/images/catalog/stools/' . $product['image'] ?> alt=<?= $product['name'] ?>>
          </div>
